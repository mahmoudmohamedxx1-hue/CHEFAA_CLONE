#!/usr/bin/env python3
"""
Fixed script to consolidate all extracted Hair Care product data from Chefaa.com
...
def main():
    ...
    for filepath in extracted_files:
        ...
        if 'key_points' in data and isinstance(data['key_points'], list):
            # Check if this is a simple list or list of products
            if data['key_points'] and isinstance(data['key_points'][0], dict):
                products = data['key_points']
                print(f"  📊 Found {len(products)} products in key_points")
        elif 'products' in data:
            products = data['products']
            print(f"  📊 Found {len(products)} products in products array")
        else:
            print(f"  ⚠️ Unknown format in {filepath}, checking for products...")
