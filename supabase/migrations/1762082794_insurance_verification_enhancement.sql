-- Migration: insurance_verification_enhancement
-- Created at: 1762082794
...
CREATE TABLE IF NOT EXISTS insurance_claims (
  ...
  processed_by TEXT, -- admin user or system
...
CREATE TABLE IF NOT EXISTS insurance_coverage_plans (
  ...
  copay_structure JSONB, -- Dynamic copay rules
  formulary_tier JSONB, -- Medication tier system
  restrictions TEXT[], -- Prior auth, step therapy, etc.
...
CREATE TABLE IF NOT EXISTS insurance_formulary (
  ...
  quantity_limit_period TEXT, -- day, month, year
...
CREATE TABLE IF NOT EXISTS insurance_prior_authorizations (
  ...
  medication_name TEXT NOT NULL,
  medication_name_ar TEXT,
  indication TEXT, -- Medical indication
  ...
  decision_made_by TEXT, -- MD, pharmacist, AI system
...
SELECT 
  ...
  END,
  CASE 
    WHEN ip.provider_code = 'EG08' THEN 5.00  -- Lowest copays for ECU
...
WHERE ip.provider_code LIKE 'EG%'
  AND random() < 0.8; -- 80% coverage rate for realistic simulation
...
-- ============================================
-- 11. STORED PROCEDURES FOR COMPLEX CALCULATIONS
-- ============================================

-- Procedure to calculate copay based on medication tier and insurance rules
...
BEGIN
  -- Determine copay percentage based on provider and tier
  ...
  -- Default calculation based on tier
...
-- Procedure to validate insurance policy
CREATE OR REPLACE FUNCTION validate_insurance_policy(
  p_provider_code TEXT,
...
BEGIN
  ...
  SELECT coverage_details INTO provider_config
  ...
  -- Basic validation logic (in real implementation, this would call provider APIs)
  IF length(p_policy_number) < 8 OR length(p_member_id) < 6 THEN
    RETURN json_build_object(
      'valid', false,
      'error', 'Invalid policy or member format'
    );
  END IF;
  ...
  RETURN json_build_object(
    ...
    'coverage_details', provider_config,
    'validated_at', NOW()
  );
END;
$$ LANGUAGE plpgsql;
