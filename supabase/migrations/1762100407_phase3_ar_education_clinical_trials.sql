-- Migration: phase3_ar_education_clinical_trials
-- Created at: 1762100407
...
CREATE TABLE IF NOT EXISTS clinical_trials (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  trial_identifier VARCHAR(100) UNIQUE NOT NULL,
  ...
  matching_score_multiplier DECIMAL(3,2) DEFAULT 1.0,
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS trial_matches (
  ...
  recommended_priority VARCHAR(20) DEFAULT 'normal',
  user_interest_level VARCHAR(20),
...
CREATE POLICY "Public can view active AR content" ON ar_content_library FOR SELECT USING (is_active = true);
CREATE POLICY "Users can view own education progress" ON user_education_progress FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Users can update own education progress" ON user_education_progress FOR UPDATE USING (auth.uid() = user_id);
CREATE POLICY "Users can insert own education progress" ON user_education_progress FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Public can view medication tutorials" ON medication_tutorials FOR SELECT USING (true);
CREATE POLICY "Users can insert own AR sessions" ON ar_session_analytics FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can view own AR sessions" ON ar_session_analytics FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Public can view active clinical trials" ON clinical_trials FOR SELECT USING (status IN ('recruiting', 'active'));
CREATE POLICY "Users can view own trial matches" ON trial_matches FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Users can update own trial matches" ON trial_matches FOR UPDATE USING (auth.uid() = user_id);
CREATE POLICY "Users can insert own trial matches" ON trial_matches FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can view own trial applications" ON trial_applications FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Users can insert own trial applications" ON trial_applications FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can update own trial applications" ON trial_applications FOR UPDATE USING (auth.uid() = user_id);
CREATE POLICY "Public can view public testimonials" ON trial_participant_experiences FOR SELECT USING (is_public = true);
CREATE POLICY "Users can insert own testimonials" ON trial_participant_experiences FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can view own clinical profile" ON user_clinical_profile FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Users can insert own clinical profile" ON user_clinical_profile FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can update own clinical profile" ON user_clinical_profile FOR UPDATE USING (auth.uid() = user_id);
...
('NCT05890123', 'Breakthrough Alzheimer Treatment Study', 'Phase II trial for novel immunotherapy targeting amyloid plaques in early-stage Alzheimer disease', 'Phase II', 'recruiting', 'Alzheimer Disease', ARRAY['alzheimers', 'dementia', 'cognitive decline'], 'drug', 'ALZ-IMMUNE-01', 'Neuroscience Institute', '{"age": "55-85", "diagnosis": "Early Alzheimer Disease", "mmse": "20-26"}'::jsonb,
